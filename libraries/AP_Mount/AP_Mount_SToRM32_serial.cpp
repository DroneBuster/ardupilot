    ...
    switch(get_mode()) {
        ...
        case MAV_MOUNT_MODE_GPS_POINT:
            if(_frontend._ahrs.get_gps().status() >= AP_GPS::GPS_OK_FIX_2D) {
                ...
                _angle_ef_target_rad.z = wrap_PI(_angle_ef_target_rad.z + _frontend._encoder.get_primary_angle() - wrap_PI(ToRad(_current_angle.z/100.0f)) - wrap_PI(_frontend._ahrs.yaw));
...
void AP_Mount_SToRM32_serial::status_msg(mavlink_channel_t chan)
{
    ...
    mavlink_msg_mount_status_send(chan, 0, 0, _current_angle.y, _current_angle.x, _current_angle.z, ToDeg(_angle_ef_target_rad.y)*100,
            ToDeg(_angle_ef_target_rad.x)*100, ToDeg(_angle_ef_target_rad.z)*100,);
}
